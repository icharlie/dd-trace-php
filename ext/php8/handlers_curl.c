...
static void dd_inject_distributed_tracing_headers(zval *ch) {
    ...
    if (ddtrace_fetch_prioritySampling_from_root(&sampling_priority)) {
        add_assoc_long(&headers, "x-datadog-sampling-priority", sampling_priority);
    }
    if (DDTRACE_G(trace_id)) {
        add_assoc_str(&headers, "x-datadog-trace-id", ddtrace_span_id_as_string(DDTRACE_G(trace_id)));
    }
    if (DDTRACE_G(span_ids_top)) {
        add_assoc_str(&headers, "x-datadog-parent-id", ddtrace_span_id_as_string(DDTRACE_G(span_ids_top)->id));
    }
    if (DDTRACE_G(dd_origin)) {
        add_assoc_str(&headers, "x-datadog-origin", DDTRACE_G(dd_origin));
    }

    zend_function *setopt_fn = zend_hash_str_find_ptr(EG(function_table), ZEND_STRL("curl_setopt"));
    zend_constant *http_header_const = zend_hash_str_find_ptr(EG(zend_constants), ZEND_STRL("CURLOPT_HTTPHEADER"));
    ...
    ZVAL_COPY(ZEND_CALL_ARG(call, 2), &http_header_const->value);
